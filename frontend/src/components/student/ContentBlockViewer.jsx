...
function VideoPlayer({ video, title }) {
  const url = video?.url || "";
  const isLocalUpload = video?.type === "upload" || url?.startsWith("/uploads");
  const videoSrc = isLocalUpload 
    ? `${import.meta.env.VITE_API_URL || "/api"}`.replace(/\/api$/, "") + url
    : url;
  ...
  const youtubeId = video?.provider === "youtube" ? getYoutubeId(url) : getYoutubeId(url);
  const isVimeo = video?.provider === "vimeo" || url?.includes("vimeo.com");
  ...
  return (
    <div
      ...
      dangerouslySetInnerHTML={{
        __html: html || "<p>No text content available.</p>",
      }}
    />
  );
}

export default function ContentBlockViewer({ block, onComplete, isPreview = false, language = "en" }) {
  ...
  const loadQuiz = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `/student/courses/${courseId}/quizzes/${block.quizId}`,
        { params: { language } },
      );
      setQuizData(data);
    } catch {
  ...
  }, [block?.quizId, block?.type, loadQuiz]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await axios.post(
        `/student/courses/${courseId}/quizzes/${block.quizId}/submit`,
        {
          courseId,
  ...
  if (block.type === "VIDEO") {
    const title = getLocalizedValue(block.title, language, "Video Lesson");
    return (
      <div className="space-y-6">
        <VideoPlayer video={block.video || { url: block.videoUrl }} title={title} />
        {block.textContent && (
          <TextContentViewer html={getLocalizedValue(block.textContent, language)} />
        )}
      </div>
    );
  }
  ...
  if (block.type === "TEXT") {
    const title = getLocalizedValue(block.title, language, "Lesson Content");
    const content = getLocalizedValue(block.textContent, language);
    return (
      <div className="space-y-6">
  ...
  if (block.type === "QUIZ") {
    ...
    if (quizError) {
      return (
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-center text-rose-600">
          ...
          <button
            onClick={loadQuiz}
            className="mt-2 text-sm font-black underline uppercase tracking-widest"
          >
            Retry
          </button>
        </div>
    ...
    const title = getLocalizedValue(block.title, language, "Quiz");
    ...
    return (
      <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-[#161b22]">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4 dark:border-slate-800 mb-6">
          ...
          {result && (
            <span
              className={`rounded-lg px-3 py-1 text-xs font-black uppercase tracking-widest ${result.status === "PASSED" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
            >
          ...
          <div className="space-y-6">
            {quizData.questions.map((question, index) => (
              <div
                ...
                className="rounded-2xl bg-slate-50/50 p-5 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800"
              >
                <p className="font-bold text-slate-900 dark:text-white mb-4">
                  <span className="text-brand-600 mr-2">Q{index + 1}.</span>
                  {getLocalizedValue(question.questionText, language, "Question")}
                </p>
                <div className="grid gap-3">
                  {(question.options || []).filter(opt => getLocalizedValue(opt.text, language).trim() !== "").map((option) => {
                    const isSelected =
                      String(selectedAnswers[question.questionId]) ===
                    ...
                    return (
                      ...
                      >
                        {getLocalizedValue(option.text, language, "Option")}
                      </button>
                    );
                  })}
                </div>
            ...
            {!result && (
              ...
              >
                {isPreview
                  ? "Quiz Submission Disabled (Preview)"
                  : submitting
                    ? "Submitting..."
                    : "Submit Quiz"}
              </button>
          ...
          !loadingQuiz && (
            <div className="py-10 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
              <p className="text-slate-500 font-medium">
                Quiz questions are not yet available for this block.
              </p>
            </div>
          )
        )}
  ...
  return (
    <TextContentViewer
      html={
        getLocalizedValue(block.textContent, language)
      }
    />
  );
}
